  <template>
    <div class="file-input-box fill-width">
      <div class="wrapper-file-input">
        <div class="input-box" @click="openFileInput">
          <h4>
            <v-icon class="mdi mdi-upload" color="grey-darken-4"></v-icon>
    ...
    async handleFileChange(event) {
      ...
      for (let i = 0; i < fileList.length; i++) {
        ...
        try {
          ...
          const response = await fetch(`${import.meta.env.VITE_TEMP_URL}/upload`, formData, {
            method: "POST",
          });

          // Handle the extracted text from OCR
          const data = await response.json()
          console.log(data.extractedText); // Display OCR result
          alert('Text Extracted: ' + data.extractedText);

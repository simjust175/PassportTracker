<template v-slot:default="{ prev, next }">
  <v-card min-width="800" class="rounded-lg" elevation="4" fluid>
    <v-btn @click="clearForm()" prepend-icon="mdi-delete" color="red"
      >Clear</v-btn
    >
    <v-btn
      @click="console.log('object in logg', info)"
    ...
    <v-stepper v-model="step" elevation="0" alt-labels>
      ...
      <v-stepper-header class="shadow-none overflow-hidden py-5" elevation="0">
        <v-divider
          :thickness="12"
          class="border-opacity-50 mt-11"
          inset
          :color="dividerColorFirst"
        />
        ...
        <v-divider
          :thickness="12"
          class="border-opacity-50 mt-11"
          :color="dividerColorSecond"
        ></v-divider>

        <v-stepper-item
          value="2"
          class="second opacity-100"
          :complete="complete.second"
          :color="dividerColorSecond"
          icon="mdi-passport"
        >
        ...
        <v-divider
          :thickness="12"
          class="border-opacity-50 mt-11"
          :color="dividerColorThird"
        ></v-divider>

        <v-stepper-item
          value="3"
          class="third opacity-100"
          :complete="complete.third"
          :color="dividerColorThird"
      ...
      <div class="px-10">
        ...
        <v-stepper-window>
          <v-stepper-window-item value="1">
            ...
            <v-sheet class="d-flex justify-center pt-3">
              <holder-info
                @selection="info.user_name = $event"
                @success="markStepAsComplete(1)"
              />
            </v-sheet>
        ...
        <v-stepper-window>
          <v-stepper-window-item value="2">
            ...
            <v-sheet>
              <document-info
                @selection="info.document = $event"
                @success="markStepAsComplete(2)"
              />
            </v-sheet>
          </v-stepper-window-item>
        </v-stepper-window>
...
const itemDefaultColor = () => {
  ...
  const vStepperItems = document.querySelectorAll(
...
//const itemColor = (stepNumber)=> step.value >= stepNumber ? '#5cb85c' : '#d8e1e7'
const dividerColorFirst = computed(() =>
  step.value >= 0 ? "#5cb85c" : "#d8e1e7"
);
const dividerColorSecond = computed(() =>
  step.value >= 1 ? "#5cb85c" : "#d8e1e7"
);
const dividerColorThird = computed(() =>
  step.value >= 2 ? "#5cb85c" : "#d8e1e7"
);
...
const disabledStat = (num) => {
  console.log("num? in function disabledStat", num)
...
const clearForm = () => {
  console.log("BEFORE reset: Info", info);
  // Reset form data
  Object.assign(info, {
  ...
  console.log("Object reset: Info", info);
  ...
  console.log("Object reset: Info", complete);
  // Reset step to the first step (step 0)
  step.value = 0;
